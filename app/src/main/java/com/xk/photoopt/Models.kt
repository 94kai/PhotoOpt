...
enum class Quality(val title: String, val caption: String, val edge: Int, val jpeg: Int, val videoEdge: Int, val bitrate: Int) {
    ...
    COMPACT("更省空间", "长边 1920 · 适合手机看", 1920, 76, 1280, 2_000_000)
}
...
data class MediaEntry(
    ...
    fun destination(prefix: String): File {
        ...
        if (copyOriginal) return original
        return if (File(source).extension.lowercase() in MediaFormats.videos) File(original.parentFile, original.nameWithoutExtension + ".mp4") else if (original.extension.lowercase() !in setOf("jpg", "jpeg")) File(original.parentFile, original.nameWithoutExtension + ".jpg") else original
    }
}
