...
| :---: | :---: | :---: |
| <a href="docs/screenshots/prepare-v1.9.6.png"><img src="docs/screenshots/prepare-v1.9.6.webp" alt="准备页" width="240" /></a> | <a href="docs/screenshots/files-v1.9.6.png"><img src="docs/screenshots/files-v1.9.6.webp" alt="文件页" width="240" /></a> | <a href="docs/screenshots/tasks-v1.9.6.png"><img src="docs/screenshots/tasks-v1.9.6.webp" alt="结果页" width="240" /></a> |
...
5. **清理已确认的原件**：只删除已确认备份且小图符合预期的手机原件。使用应用内“清理原件”时，每个目录会保留最新一张图片及其同名实况配对，避免相册被隐藏。
...
- **并非所有文件都能处理**：普通 JPEG、静态 PNG/WebP 和符合条件的 SDR 视频可处理；HEIC/HEIF 可在兼容模式下提取静态主图并转为 JPEG；Apple Live Photo 的动态保留、RAW、HDR 视频等暂不支持，厂商实况兼容性因机型而异。请检查失败和跳过项。
...
**每个目录保留最新一张图片，避免相册被隐藏。** 优先按拍摄时间判断，缺失时按文件修改时间；同名实况视频一起保留。检查会包含同目录中未纳入扫描清单的图片，确认页列出保留文件和原因，预计清理数量、空间不包含这些文件。删除前再次检查；保留图片被移走或替换时，会重新选择要保留的图片。
